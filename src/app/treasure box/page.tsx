...
/* ═══════════════════════════════════════════════
   GEM + color helpers (for success screen)
═══════════════════════════════════════════════ */
function blendHex(hex: string, t: number): string {
  const r = parseInt(hex.slice(1, 3), 16), g = parseInt(hex.slice(3, 5), 16), b = parseInt(hex.slice(5, 7), 16);
  if (t >= 0) return `rgb(${Math.round(r + (255 - r) * t)},${Math.round(g + (255 - g) * t)},${Math.round(b + (255 - b) * t)})`;
  const f = 1 + t;
  return `rgb(${Math.round(r * f)},${Math.round(g * f)},${Math.round(b * f)})`;
}
function GemIcon({ color, size = 44 }: { color: string; size?: number }) {
  return (
    <svg viewBox="0 0 40 44" width={size} height={size} fill="none">
      <polygon points="20,4 32,14 8,14" fill={color} stroke="rgba(255,255,255,0.4)" strokeWidth="0.8" />
      <polygon points="8,14 20,4 12,26" fill={blendHex(color, -0.12)} stroke="rgba(255,255,255,0.3)" strokeWidth="0.8" />
      <polygon points="32,14 20,4 28,26" fill={blendHex(color, 0.12)} stroke="rgba(255,255,255,0.3)" strokeWidth="0.8" />
      <polygon points="8,14 32,14 28,26 12,26" fill={color} stroke="rgba(255,255,255,0.25)" strokeWidth="0.8" />
      <polygon points="12,26 20,40 8,14" fill={blendHex(color, -0.20)} stroke="rgba(255,255,255,0.2)" strokeWidth="0.8" />
      <polygon points="28,26 20,40 32,14" fill={blendHex(color, 0.08)} stroke="rgba(255,255,255,0.2)" strokeWidth="0.8" />
      <polygon points="12,26 28,26 20,40" fill={blendHex(color, -0.15)} stroke="rgba(255,255,255,0.2)" strokeWidth="0.8" />
      <polygon points="16,8 22,8 19,13" fill="rgba(255,255,255,0.5)" />
    </svg>
  );
}
...
export default function TreasureBoxPage() {
  ...
  if (screen === "success") return (
    <div style={{ position: "fixed", inset: 0, background: "linear-gradient(160deg,#0E0820 0%,#1A0C38 50%,#0A1828 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-nunito)" }}>
      <style>{`@keyframes gemPop{0%{transform:scale(0.4) rotate(-15deg);opacity:0}60%{transform:scale(1.15);opacity:1}100%{transform:scale(1);opacity:1}} @keyframes fadeUp{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <div style={{ animation: "gemPop 0.7s cubic-bezier(.36,.07,.19,.97) both", marginBottom: 28 }}>
        <GemIcon color={selDef?.col ?? "#8B5CF6"} size={72} />
      </div>
      <div style={{ width: 120, height: 1.5, background: "linear-gradient(90deg,transparent,rgba(200,160,80,0.7),transparent)", marginBottom: 24, animation: "fadeUp 0.5s ease 0.3s both", opacity: 0 }} />
      <p style={{ fontFamily: "var(--font-playfair), Georgia, serif", fontSize: "clamp(22px,4vw,30px)", fontWeight: 700, color: "#F5DFA0", margin: "0 0 8px", animation: "fadeUp 0.5s ease 0.4s both", opacity: 0 }}>
        +10 seeds added 🌱
      </p>
      <p style={{ fontSize: 13, color: "rgba(210,185,120,0.6)", fontStyle: "italic", margin: "0 0 48px", animation: "fadeUp 0.5s ease 0.5s both", opacity: 0 }}>
        You showed up and felt it all — that takes courage.
      </p>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, animation: "fadeUp 0.5s ease 0.6s both", opacity: 0 }}>
        ...
        <button onClick={() => { setScreen("main"); setEmotion(null); setBodyPart(null); setFeelType(null); setLevel(null); }} style={{ background: "none", border: "none", color: "rgba(180,155,220,0.45)", fontSize: 13, cursor: "pointer", fontStyle: "italic", fontFamily: "var(--font-nunito)" }}>
          Check in again
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "linear-gradient(160deg,#080418 0%,#110830 50%,#080C24 100%)", overflowY: "auto", fontFamily: "var(--font-nunito)" }}>
      <style>{`
        ...
        .fci-done-btn { width:100%; max-width:660px; padding:15px; border-radius:18px; border:none; background:linear-gradient(135deg,#6D28D9,#A855F7); color:#fff; font-size:16px; font-weight:800; font-family:var(--font-baloo); cursor:pointer; box-shadow:0 4px 22px rgba(109,40,217,0.55); letter-spacing:0.04em; }
        .fci-done-btn:disabled { opacity:0.3; cursor:not-allowed; }
      `}</style>

      <div style={{ width: "100%", maxWidth: 720, margin: "0 auto", padding: "16px 14px 40px", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        ...
        {/* Row 1: Happy Angry Calm Sad Worried */}
        ...
        {/* Row 2: Silly Tired Confused Anxious Excited */}
        <div className="fci-emo-row">
